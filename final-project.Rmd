...
news_train$title_character_length <- lapply(news_train$cleaned_title, nchar)#Creates varible of length of title
...
#news_train$text_sentiment <- sentiment(news_train$cleaned_text)$sentiment #Returns emotional sentiment of article with numeric score 
...
```{r}
names(news_train)[names(news_train)== "label"] <- "fake" #renaming dependent varible for readability purposes
```
...
#news_test$text_sentiment <- sentiment(news_test$cleaned_text)$sentiment #Returns emotional sentiment of article with numeric score 
